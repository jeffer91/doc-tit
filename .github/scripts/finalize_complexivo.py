...
assert old in s, "BODY no encontrado"
s=s.replace(old,'''  const BODY = window.DOC_TIT_COMPLEXIVO_PDF?.config?.layout?.body || {
    left: 72,
    right: 72,
    top: 108,
    bottom: 62,
...
assert anchor in s, "lowerPeriod no encontrado"
...
assert needle in s, "metadatos PDF no encontrados"
...
assert pat.search(s), "paragraph no encontrado"
para='''    function paragraph(text,opts={}){
...
assert rpat.search(s), "reserveIndexPages no encontrado"
...
oldapi='''      formatDateShort,formatDateLong,lowerPeriod,normalize,totals,
      insertSectionImage,reference,drawVerticalBars,drawGroupBars,drawTimeline,
      getAnalysisSentences,imageFormat,drawTOCPage,'''
assert oldapi in s, "sectionApi no encontrado"
s=s.replace(oldapi,'''      formatDateShort,formatDateLong,lowerPeriod,normalize,totals,joinNatural,policy,
      insertSectionImage,reference,drawVerticalBars,drawGroupBars,drawTimeline,
...
assert sig in s, "signatureBlock call no encontrado"
...
'''
assert start in s, "generatePdf validation no encontrada"
validation='''  function validateGenerationData(data){
    const errors=[];
    const warnings=[];
    ...
    (data.schedule||[]).forEach(r=>{
      if(r.start&&r.end&&r.start>r.end) errors.push("La fecha de inicio no puede ser posterior a la fecha fin en: "+r.activity+".");
    });
    const scheduled=(data.schedule||[]).filter(r=>r.start);
    for(let i=1;i<scheduled.length;i++){
      if(scheduled[i].start<scheduled[i-1].start){
        errors.push("El cronograma debe mantener las actividades en orden cronológico.");
        break;
      }
    }
...
'''
s=s.replace(start,validation,1)

ctx='''        analysis:data.analysis,
        institutional:state.institutional,
        code'''
assert ctx in s, "contexto de PDF no encontrado"
s=s.replace(ctx,'''        analysis:data.analysis,
        institutional:state.institutional,
        meta:{
          version:doc.version||window.DOC_TIT_COMPLEXIVO_PDF?.config?.policy?.version||"1.0",
...
assert layout in s, "layout script no encontrado"
if 'pdf/config/policy.js' not in s:
    s=s.replace(layout,layout+'\n  <script src="pdf/config/policy.js?v=20260904-complete-1"></script>',1)
evaluation='<script src="pdf/sections/evaluation.js?v=20260904-complete-1"></script>'
assert evaluation in s, "evaluation script no encontrado"
if 'pdf/sections/closure.js' not in s:
    s=s.replace(evaluation,evaluation+'\n  <script src="pdf/sections/closure.js?v=20260904-complete-1"></script>\n  <script src="pdf/sections/general-summary.js?v=20260904-complete-1"></script>',1)
bibliography='<script src="pdf/sections/bibliography.js?v=20260904-complete-1"></script>'
assert bibliography in s, "bibliography script no encontrado"
if 'pdf/sections/annexes.js' not in s:
    s=s.replace(bibliography,bibliography+'\n  <script src="pdf/sections/annexes.js?v=20260904-complete-1"></script>',1)
