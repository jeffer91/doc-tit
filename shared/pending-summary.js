(() => {
  ...
  function sectionDef(id){return manifest.sections?.find(item=>item.id===id)||{id,label:id,title:id};}
  function templateKey(card){return $("[data-u]",card)?.dataset.u||$("[data-d]",card)?.dataset.d||$("[data-v]",card)?.dataset.v||"";}
  function templateStatus(card){return String($(".ptstatus",card)?.textContent||"").trim();}
  function hasScheduleSets(){return !!window.DOC_TIT_MULTI_SCHEDULES?.hasSchedules?.();}
  function scheduleSection(){return manifest.templateSections?.cronograma||"";}
  function sourceButton(attr,key){return $$(`#ptcenter [data-${attr}]`).find(button=>button.dataset[attr]===key)||null;}
  function clickSource(attr,key){const source=sourceButton(attr,key);if(source&&!source.disabled)source.click();}
  function openSection(section){
    const tab=$(`.svd-section-tabs [data-svd-section="${section}"]`);if(tab){tab.click();return true;}
    const more=$(`.svd-section-more [data-svd-more-section="${section}"]`);if(more){more.click();return true;}
    return false;
  }
  function viewTemplate(key,section){openSection(section);window.setTimeout(()=>clickSource("v",key),140);}
  ...
  function pendingTemplates(){
    return $$("#ptcenter .ptcard").map(card=>{
      ...
      if(key==="cronograma"&&hasScheduleSets())return null;
  ...
  function pendingSections(represented){
    return $$(".svd-section-tabs [data-svd-section]").map(tab=>{
      ...
      if(hasScheduleSets()&&id===scheduleSection())return null;
  ...
  function stateReady(){const button=$("#generateBtn")||$("#generateBtnBottom");return !!button&&!button.disabled;}
  function diagnostic(){$("#docCoreReviewBtn")?.click();}

  function ensurePanel(){
    const root=$("#referenceView");if(!root)return null;
    let panel=$("#referencePendingPanel");
    if(!panel){panel=document.createElement("section");panel.id="referencePendingPanel";panel.className="reference-pending-panel";const complete=$("#referenceDocumentCard");if(complete?.parentElement===root)complete.after(panel);else root.appendChild(panel);}
    return panel;
  }

  function render(){
    timer=null;const panel=ensurePanel();if(!panel)return;
    const templates=pendingTemplates();
    const represented=new Set(templates.map(item=>item.section));if(hasScheduleSets()&&scheduleSection())represented.add(scheduleSection());
    const sections=pendingSections(represented);const items=[...templates,...sections];const ready=stateReady();const visible=activeSection()==="information"&&!ready;
    const signature=JSON.stringify({section:activeSection(),ready,hasScheduleSets:hasScheduleSets(),items});
    if(signature===lastSignature){panel.hidden=!visible;return;}lastSignature=signature;
    ...
    if(!items.length){
      ...
      $("[data-pending-diagnostic]",panel)?.addEventListener("click",diagnostic);return;
    }
    ...
    items.forEach(item=>{
      const card=document.createElement("article");card.className="reference-pending-item";
      if(item.type==="template"){
        ...
        $("[data-pending-action='upload']",card)?.addEventListener("click",()=>item.key==="cronograma"&&window.DOC_TIT_MULTI_SCHEDULES?.openImport?window.DOC_TIT_MULTI_SCHEDULES.openImport():clickSource("u",item.key));
        $("[data-pending-action='view']",card)?.addEventListener("click",()=>item.key==="cronograma"&&window.DOC_TIT_MULTI_SCHEDULES?.openImport?window.DOC_TIT_MULTI_SCHEDULES.openImport():viewTemplate(item.key,item.section));
      }else{
  ...
  function schedule(){if(timer)return;timer=window.setTimeout(render,70);}
  document.addEventListener("doc-tit:section-changed",schedule);
  document.addEventListener("doc-tit:schedules-changed",schedule);
  document.addEventListener("input",schedule,true);document.addEventListener("change",schedule,true);
  document.addEventListener("click",event=>{if(event.target?.closest("#ptapply,#applyImportBtn,#approveScheduleBtn,[data-u],[data-d],[data-v]"))window.setTimeout(schedule,120);},true);
  new MutationObserver(mutations=>{if(mutations.every(m=>m.target instanceof Element&&m.target.closest?.("#referencePendingPanel")))return;schedule();}).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:["class","hidden","disabled"]});
  schedule();
})();
